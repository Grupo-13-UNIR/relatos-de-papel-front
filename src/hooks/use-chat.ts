...
export function useChat(useAi: boolean = false) {
  const [messages, setMessages] = useState<
    { id: number; text: string; sender: string; timestamp: string; type?: string }[]
  >([]);
  const [isConnected, setIsConnected] = useState(false);
  const [isConnecting, setIsConnecting] = useState(false);
  const stompClient: RefObject<Client | null> = useRef(null);
  const clientIdRef: RefObject<string | null> = useRef(null);
  ...
  useEffect(() => {
    ...
    stompClient.current.onConnect = (frame) => {
      ...
      const topic = `/topic/support/${clientIdRef.current}`;
